...
import { AlertCircle, X, TriangleAlert, Settings as SettingsIcon, Share2, Users } from "lucide-react";
import { QRCodeSVG } from 'qrcode.react';

function Header() {
  const { 
    isConnected, isSignalLost, connectionError, setConnectionError, 
    maxLoadN, setMaxLoadN, isGuestMode, sessionId, viewerCount 
  } = useSensorStore();
    ...
    <header className="bg-surface border-b border-surface-border shadow-sm z-10">
      ...
      <div className="flex justify-between items-center px-4 sm:px-8 py-2">
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {isSignalLost && (isConnected || isGuestMode) && (
            <div className="flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg text-amber-600 shadow-sm">
              ...
              <span className="text-[10px] font-bold tracking-wider whitespace-nowrap">UTRATA SYGNAŁU</span>
            </div>
          )}

          {/* Status połączenia */}
          <div className={`flex items-center gap-2 px-4 py-2 rounded-md text-[11px] font-bold tracking-widest shadow-sm border ${isGuestMode ? 'bg-purple-500/10 text-purple-600 border-purple-500/20' : (isConnected ? (isSignalLost ? 'bg-amber-500/10 text-amber-600 border-amber-500/20' : 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20') : 'bg-red-500/5 text-red-500 border-red-500/20')}`}>
            {isGuestMode 
              ? "TRYB WIDZA (P2P)"
              : (isConnected 
                ? (isSignalLost ? "BRAK DANYCH" : "PODŁĄCZONO PICO") 
                : "NIE PODŁĄCZONO")}
          </div>
          ...
          <div className="flex items-center gap-2">
            {!isGuestMode && (
              ...
              >
                {sessionId ? (
                  <>
                    <Users className="w-4 h-4 animate-pulse" /> WIDZÓW: {viewerCount}
                  </>
                ) : (
                  <>
        ...
        <div className="flex md:hidden items-center gap-2 sm:gap-3">
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] font-bold tracking-wider shadow-sm border ${isGuestMode ? 'bg-purple-500/10 text-purple-600 border-purple-500/20' : (isConnected ? (isSignalLost ? 'bg-amber-500/10 text-amber-600 border-amber-500/20' : 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20') : 'bg-red-500/5 text-red-500 border-red-500/20')}`}>
            <span className={`w-1.5 h-1.5 rounded-full bg-current ${!isSignalLost && (isConnected || isGuestMode) ? 'animate-pulse shadow-[0_0_6px_currentColor]' : ''}`}></span>
            {isGuestMode ? "WIDZ" : (isConnected ? (isSignalLost ? "BRAK" : "OK") : "OFF")}
          </div>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-surface-border bg-surface px-4 py-3 flex flex-col gap-3 animate-in slide-in-from-top">
          {!isGuestMode && (
            ...
            >
              <Share2 className="w-4 h-4" /> {sessionId ? `UDOSTĘPNIONO (${viewerCount} widzów)` : 'UDOSTĘPNIJ SESJĘ'}
            </button>
          )}
      ...
      {showShareModal && createPortal(
        <div className="fixed inset-0 z-500 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in" onClick={() => setShowShareModal(false)}>
          <div className="bg-surface w-full max-w-md rounded-2xl shadow-2xl p-6 sm:p-8 border border-surface-border flex flex-col gap-6 animate-zoom-in relative" onClick={e => e.stopPropagation()}>
            <button onClick={() => setShowShareModal(false)} className="absolute top-4 right-4 p-2 text-slate-400 hover:text-red-500 transition-colors rounded-full hover:bg-slate-100">
              <X className="w-5 h-5" />
            </button>

            <div className="text-center">
              <div className="w-12 h-12 bg-purple-500/10 text-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Share2 className="w-6 h-6" />
              </div>
              <h3 className="font-bold text-xl text-brand-text mb-2">Udostępnij Sesję Na Żywo</h3>
              <p className="text-xs text-brand-secondary">
                Zeskanuj kod QR lub użyj linku, aby połączyć się z urządzeń mobilnych (Peer-to-Peer).
              </p>
            </div>
            ...
            {isStartingHost ? (
              ...
              <>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Kod Sesji</div>
                  <div className="font-mono text-lg font-bold text-brand-primary tracking-widest">{sessionId}</div>
                </div>
              ...
              <div className="text-center text-red-500 py-4 font-bold">Wystąpił błąd podczas generowania sesji.</div>
            )}
          </div>
        </div>,
        document.body
